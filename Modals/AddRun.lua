...
function addRun(dungName, level, status, affixes, time, runDate, spec, character, role, party, deaths, note)
    -- Determine the dungeon, level, and affixes if not provided
    ...
    -- If time is not provided, assume 00:00:00.00
    if not time then
        time = "00:00:00.00"
    end
    
    -- If date is not provided, assume now
    if not runDate then
        runDate = date("%Y-%m-%d %H:%M:%S")  -- Use os.date() to format the current date and time
    end

    -- If status is not provided, assume 'started'
    if not status then
        status = "started"
    end
    ...
    -- If status is not provided, assume player's role
    if not role then
        role = GetPlayerRoleFromSpec()
    end

    -- If status is not provided, assume player's character
    if not character then
        local playerName, playerRealm = UnitName("player")
        character = playerRealm and (playerName .. "-" .. playerRealm) or playerName
    end

    -- spec is already assumed to be the player's current spec (passed parameter)
    if not spec then
    ...
    -- should not have deaths for fresh run
    -- if not deaths then
    ...
    -- Add the run data to the runsDB table
    table.insert(runsDB, {
        spec = spec,
        id = runID,
